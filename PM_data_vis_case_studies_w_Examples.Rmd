...
ggplot(fish, aes(x=year, y=mercury)) + geom_boxplot() +
   theme_minimal() +
  # make text larger 
  theme(text=element_text(size=12)) + 
  # set a title 
  ggtitle("Fish mercury concentration at two sample periods")

ggplot(fish, aes(x=length, y=mercury, col=year)) + geom_point() +
  theme_minimal() +
  # make text larger 
  theme(text=element_text(size=12)) + 
  # set a title 
  ggtitle("Fish length (cm) v. mercury content (ug)")
...
ggplot(temps, aes(x=year_month)) +
     geom_line(aes(y=max_temp_f), col="red") +
     geom_line(aes(y=min_temp_f), col="blue") +
    theme_minimal() +
  # make text larger 
    theme(text=element_text(size=12)) + 
  # set a title 
    ggtitle("Monthly average min and max temperature (F)")
...
temps %>% dplyr::filter(year==1940 | year==2020) %>% 
  ...
  theme(text=element_text(size=12)) + 
  # set a title 
  ggtitle("Comparing monthly average temperature, 1940 to 2020")

temps %>% filter(month==6) %>% 
  ggplot(aes(x=year, y=min_temp_f)) +
  geom_point() + 
  ...
  theme(text=element_text(size=12)) + 
  # set a title 
  ggtitle("Average minimum temperature in June")
...
ggplot(inat, aes(x=place_state_name, y=count)) +
  geom_boxplot()


ggplot(inat, aes(y=common_name, 
                     x=count, 
  ...
  theme(text=element_text(size=12)) + 
  # set a title 
  ggtitle("Count of threatened fish species observed in iNaturalist in MN/WI")
